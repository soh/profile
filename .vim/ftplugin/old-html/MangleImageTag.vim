...
function! s:ImageSize(image) "{{{1
    ...
    let image = a:image
    " strip of the first slash if we can't find the image
    " often in HTML images are prepended with a slash
	if filereadable(image) != 1
        let image = fnamemodify(image, ':s?/??')
    endif

	if filereadable(image)
		let ldsave=&lazyredraw
		set lazyredraw

		new
		silent exe '$read ' . image
		go
